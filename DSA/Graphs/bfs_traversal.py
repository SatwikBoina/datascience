# getting to the nodes level wise
...
class Graph :
    ...
    def bfs_traversal(self,start):
        ...
        def _bfs(start,is_visited,bfs_result,deque_list):
            if is_visited[start]==True:
                return
            #if is_visited[start]==False:
            print(start)
            start_index = self.get_index(start)
            ...
            for i in range(len(self.vertices)):
                if  start_index== i:
                    continue
                if self.adj_matrix[start_index][i] == 1 and is_visited[self.vertices[i]] == False:
                    #bfs_result.append(self.vertices[i])
                    deque_list.append(self.vertices[i])
            if len(deque_list)!=0:
                _bfs(deque_list.popleft(),is_visited,bfs_result,deque_list)  

        _bfs(start,is_visited,bfs_result,deque_list)     
        return bfs_result
...
print(graph_demo.dfs_traversal("1"))
print(graph_demo.bfs_traversal('1'))